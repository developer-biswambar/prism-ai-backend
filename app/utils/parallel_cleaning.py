...
class ParallelDataCleaner:
    ...
    def clean_dataframe_parallel(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
        ...
        logger.info(
            f"🚀 Starting parallel data cleaning for {original_shape[0]:,} rows × {original_shape[1]} columns using {self.max_workers} threads")
        ...
        if empty_columns:
            logger.info(
                f"  🗑️  Removing {len(empty_columns)} empty columns: {empty_columns[:5]}{'...' if len(empty_columns) > 5 else ''}")
