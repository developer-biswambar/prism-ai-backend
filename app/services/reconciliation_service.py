...
class OptimizedFileProcessor:
    ...
    def _process_batch_parallel(self, batch_data: Dict) -> Dict:
        ...
        Supports different matching modes: one_to_one, one_to_many, many_to_one, many_to_many
        ...
        match_mode = batch_data.get('match_mode', 'many_to_many')
        global_matched_a = batch_data.get('matched_indices_a', set())
        global_matched_b = batch_data.get('matched_indices_b', set())
        ...
        try:
            for idx_a, row_a in batch_a.iterrows():
                orig_index_a = row_a['_orig_index_a']
                match_key = row_a['_match_key']
                
                # Check if this A record should be skipped based on match mode
                if match_mode in ['one_to_one', 'one_to_many'] and orig_index_a in global_matched_a:
                    continue  # Skip already matched A records for one_to_* modes

                # Get potential matches from grouped data
                if match_key in grouped_b:
                    potential_matches = grouped_b[match_key]
                    found_match_for_a = False

                    for idx_b, row_b in potential_matches.iterrows():
                        orig_index_b = row_b['_orig_index_b']
                        
                        # Check if this B record should be skipped based on match mode
                        if match_mode in ['one_to_one', 'many_to_one'] and orig_index_b in global_matched_b:
                            continue  # Skip already matched B records for *_to_one modes
                        ...
                        if all_rules_match:
                            ...
                            matched_indices_a.add(orig_index_a)
                            matched_indices_b.add(orig_index_b)
                            found_match_for_a = True
                            
                            # Apply match mode constraints
                            if match_mode == 'one_to_one':
                                # Both A and B can only match once - break both loops
                                break
                            elif match_mode == 'one_to_many':
                                # Each A can match only one B, but B can match many A
                                break  # A found its match, move to next A
                            elif match_mode == 'many_to_one':
                                # A can match many B, but each B can only match once
                                # Continue to find more B matches for this A
                                pass
                            # many_to_many: no breaks, find all possible matches
                    
                    # For one_to_many mode, if A found any match, it's done
                    if match_mode == 'one_to_many' and found_match_for_a:
                        pass  # A can match multiple B records, so continue processing
    ...
    def reconcile_files_optimized(self, df_a: pd.DataFrame, df_b: pd.DataFrame,
        ...
        Args:
            match_mode: Matching behavior
                - "one_to_many": Each File A record matches at most one File B record, but File B records can match multiple File A records (default)
                - "many_to_one": File A records can match multiple File B records, but each File B record matches at most one File A record
                - "one_to_one": Each record matches at most once (fastest)
        ...
        logger.info(
            f"⚙️ Processing {len(df_a_work):,} records in {total_batches} batches (size: {batch_size}) using {self.max_workers} workers")
        logger.info(
            f"🖥️ Hardware optimization: {self.threading_config.server_class} ({self.threading_config.available_cores} cores, {self.threading_config.optimization_level})")

        # Use parallel processing with configured workers (minimum 1)
        effective_workers = max(1, self.max_workers)
        logger.info(f"🚀 Using parallel batch processing with {effective_workers} workers")
        ...
        for batch_idx, start_idx in enumerate(range(0, len(df_a_work), batch_size), 1):
            ...
            batch_data = {
                ...
                'matched_indices_a': matched_indices_a,  # Pass current matched indices for constraint checking
                'matched_indices_b': matched_indices_b,
                'batch_idx': batch_idx
            }
            ...
            for future in as_completed(future_to_batch, timeout=timeout):
                try:
                    ...
                    if completed_batches % 10 == 0 or completed_batches == total_batches:
                        ...
                        logger.info(
                            f"📈 Completed {completed_batches}/{total_batches} batches ({progress_pct:.1f}%) - {len(matches):,} matches found")
        ...
        if find_closest_matches:
            ...
            if len(unmatched_a) > 0 and len(full_df_b) > 0:
                logger.info(
                    f"🔍 Analyzing {len(unmatched_a):,} unmatched A records against entire File B ({len(full_df_b):,} records)")
            ...
            if len(unmatched_b) > 0 and len(full_df_a) > 0:
                logger.info(
                    f"🔍 Analyzing {len(unmatched_b):,} unmatched B records against entire File A ({len(full_df_a):,} records)")
    ...
    def _add_closest_matches(self, unmatched_source: pd.DataFrame, full_target: pd.DataFrame,
        ...
        logger.info(
            f"🚀 Starting optimized closest match analysis for {len(unmatched_source):,} unmatched records against {len(full_target):,} target records")
        ...
        logger.info(
            f"🚀 Using hardware-aware threading: {num_threads} threads on {threading_config.server_class} ({threading_config.available_cores} cores)")
        ...
        return self._add_closest_matches_optimized_batch(unmatched_source, full_target, recon_rules, source_file,
                                                         closest_match_config, num_threads)
    ...
    def _add_closest_matches_optimized_single(self, unmatched_source: pd.DataFrame, full_target: pd.DataFrame,
        ...
        for idx, source_row in unmatched_source.iterrows():
            ...
            if processed_count % 1000 == 0:
                logger.info(
                    f"📊 Processed {processed_count:,} / {len(unmatched_source):,} records | Matches: {matches_found:,} | Early exits: {early_terminations:,}")
        ...
        efficiency_pct = (
                                     skipped_comparisons / total_potential_comparisons) * 100 if total_potential_comparisons > 0 else 0
    ...
    def _add_closest_matches_optimized_batch(self, unmatched_source: pd.DataFrame, full_target: pd.DataFrame,
        ...
        # Use batch size from threading configuration
        optimal_batch_size = self.batch_size

        logger.info(f"🔧 Final Configuration: {num_processes} processes, batch size: {optimal_batch_size} (from threading config)")
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=num_processes) as executor:
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        logger.info(
                            f"📈 Completed batch {completed}/{len(batches)} ({progress_pct:.1f}%) | Elapsed: {elapsed:.1f}s | ETA: {eta:.1f}s")
            ...
            logger.info(
                f"💪 Hardware utilization: {num_processes}/{available_cores} cores ({(num_processes / available_cores) * 100:.1f}%)")
    ...
    def _add_closest_matches_ultra_optimized(self, unmatched_source: pd.DataFrame, full_target: pd.DataFrame,
        ...
        if exact_match_columns:
            ...
            for i, (source_col, target_col) in enumerate(exact_match_columns, 1):
                ...
                target_normalized[f"{target_col}_normalized"] = target_normalized[target_col].astype(
                    str).str.strip().str.lower()
                # Normalize source column
                source_normalized[f"{source_col}_normalized"] = source_normalized[source_col].astype(
                    str).str.strip().str.lower()
        else:
        ...
        for idx, source_row in unmatched_source.iterrows():
            ...
            if time.time() - loop_start_time > timeout_seconds:
                logger.error(
                    f"❌ Ultra-optimization timeout after {timeout_seconds}s - dataset too large for this method")
                raise TimeoutError(
                    f"Ultra-optimization timed out after {timeout_seconds} seconds. Consider using smaller dataset or batch processing.")
            ...
            if processed_count % 100 == 0:  # More frequent progress for large datasets
                ...
                logger.info(
                    f"📊 Processed {processed_count:,} / {len(unmatched_source):,} records | Matches: {matches_found:,} | Rate: {rate:.1f}/s | ETA: {eta:.0f}s")
        ...
        reduction_ratio = (len(full_target) - avg_comparisons_per_record) / len(full_target) * 100 if len(
            full_target) > 0 else 0
        ...
        logger.info(f"   • Early terminations: {early_terminations:,}")
        logger.info(
            f"   • Avg comparisons per record: {avg_comparisons_per_record:.1f} (vs {len(full_target):,} without optimization)")
    ...
    def _process_batch_closest_matches(self, batch_df: pd.DataFrame, full_target: pd.DataFrame,
        ...
        if use_sampling:
            ...
            logger.debug(
                f"🎯 Batch {batch_idx}: Using target sampling ({sample_size:,} records from {target_size:,} total)")
        else:
        ...
        for idx, source_row in batch_df.iterrows():
            ...
            if processed_in_batch % 50 == 0:
                ...
                logger.debug(
                    f"Batch {batch_idx}: Processed {processed_in_batch}/{len(batch_df)} records at {rate:.1f}/s")
...
class OptimizedReconciliationStorage:
    def __init__(self):
        ...
        logger.info(f"ReconciliationStorage: Initialized with uploaded_files storage for viewer compatibility")
    ...
    def store_results(self, recon_id: str, results: Dict[str, pd.DataFrame]) -> bool:
        ...
        try:
            ...
            if matched_count > 0:
                matched_columns = list(results['matched'].columns)
                matched_info = {
                    'file_id': f"{recon_id}_matched",
                    ...
                    'total_columns': len(matched_columns),
                    'columns': matched_columns,
                    'upload_time': timestamp_iso,
                    'file_size_mb': 0,  # We don't calculate file size for reconciliation results
                    'data_types': {col: str(dtype) for col, dtype in results['matched'].dtypes.items()},
                ...
                success = self.storage.save(f"{recon_id}_matched", matched_data, matched_info)
            ...
            if unmatched_a_count > 0:
                unmatched_a_columns = list(results['unmatched_file_a'].columns)
                unmatched_a_info = {
                    'file_id': f"{recon_id}_unmatched_a",
                    'filename': f'reconciliation_{recon_id}_unmatched_a.xlsx',
                    'custom_name': None,
                    'file_type': 'reconciliation',
                    'file_source': 'reconciliation',
                    'total_rows': unmatched_a_count,
                    'total_columns': len(unmatched_a_columns),
                    'columns': unmatched_a_columns,
                    'upload_time': timestamp_iso,
                    'file_size_mb': 0,
                    'data_types': {col: str(dtype) for col, dtype in results['unmatched_file_a'].dtypes.items()},
                    'reconciliation_id': recon_id,
                    'result_type': 'unmatched_a'
                }
                
                unmatched_a_data = {
                    'info': unmatched_a_info,
                    'data': results['unmatched_file_a']
                }
                
                success = self.storage.save(f"{recon_id}_unmatched_a", unmatched_a_data, unmatched_a_info)
                all_success = all_success and success
                if success:
            ...
            if unmatched_b_count > 0:
                unmatched_b_columns = list(results['unmatched_file_b'].columns)
                unmatched_b_info = {
                    'file_id': f"{recon_id}_unmatched_b",
                    'filename': f'reconciliation_{recon_id}_unmatched_b.xlsx',
                    'custom_name': None,
                    'file_type': 'reconciliation',
                    'file_source': 'reconciliation',
                    'total_rows': unmatched_b_count,
                    'total_columns': len(unmatched_b_columns),
                    'columns': unmatched_b_columns,
                    'upload_time': timestamp_iso,
                    'file_size_mb': 0,
                    'data_types': {col: str(dtype) for col, dtype in results['unmatched_file_b'].dtypes.items()},
                    'reconciliation_id': recon_id,
                    'result_type': 'unmatched_b'
                }
                
                unmatched_b_data = {
                    'info': unmatched_b_info,
                    'data': results['unmatched_file_b']
                }
                
                success = self.storage.save(f"{recon_id}_unmatched_b", unmatched_b_data, unmatched_b_info)
                all_success = all_success and success
                if success:
            ...
            elapsed = time.time() - start_time
            if all_success:
                logger.info(f"ReconciliationStorage STORE SUCCESS: {recon_id} stored as separate files in uploaded_files in {elapsed:.3f}s")
            else:
    ...
    def get_results(self, recon_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            result = {
                ...
                'unmatched_file_a': [],
                'unmatched_file_b': [],
                'row_counts': {'matched': 0, 'unmatched_a': 0, 'unmatched_b': 0},
                'timestamp': None
            }
            ...
            # Get matched results
            if self.storage.exists(f"{recon_id}_matched"):
                matched_data = self.storage.get(f"{recon_id}_matched")
                result['matched'] = matched_data['data'].to_dict('records')
                result['row_counts']['matched'] = len(result['matched'])
                result['timestamp'] = matched_data['info'].get('upload_time')
                total_records += len(result['matched'])
            
            # Get unmatched A results
            if self.storage.exists(f"{recon_id}_unmatched_a"):
                unmatched_a_data = self.storage.get(f"{recon_id}_unmatched_a")
                result['unmatched_file_a'] = unmatched_a_data['data'].to_dict('records')
                result['row_counts']['unmatched_a'] = len(result['unmatched_file_a'])
                if not result['timestamp']:
                    result['timestamp'] = unmatched_a_data['info'].get('upload_time')
                total_records += len(result['unmatched_file_a'])
            
            # Get unmatched B results
            if self.storage.exists(f"{recon_id}_unmatched_b"):
                unmatched_b_data = self.storage.get(f"{recon_id}_unmatched_b")
                result['unmatched_file_b'] = unmatched_b_data['data'].to_dict('records')
                result['row_counts']['unmatched_b'] = len(result['unmatched_file_b'])
                if not result['timestamp']:
                    result['timestamp'] = unmatched_b_data['info'].get('upload_time')
                total_records += len(result['unmatched_file_b'])
    ...
    def get_metadata_only(self, recon_id: str) -> Optional[Dict]:
        """Get only metadata from stored results by checking separate files"""
        import time
        start_time = time.time()
        ...
        try:
            ...
            # Get metadata from separate files created by new storage format
            row_counts = {'matched': 0, 'unmatched_a': 0, 'unmatched_b': 0}
            timestamp = None
            
            # Check each file for metadata
            if self.storage.exists(f"{recon_id}_matched"):
                matched_metadata = self.storage.get_metadata_only(f"{recon_id}_matched")
                if matched_metadata:
                    row_counts['matched'] = matched_metadata.get('total_rows', 0)
                    timestamp = matched_metadata.get('upload_time')
            
            if self.storage.exists(f"{recon_id}_unmatched_a"):
                unmatched_a_metadata = self.storage.get_metadata_only(f"{recon_id}_unmatched_a")
                if unmatched_a_metadata:
                    row_counts['unmatched_a'] = unmatched_a_metadata.get('total_rows', 0)
                    if not timestamp:
                        timestamp = unmatched_a_metadata.get('upload_time')
            
            if self.storage.exists(f"{recon_id}_unmatched_b"):
                unmatched_b_metadata = self.storage.get_metadata_only(f"{recon_id}_unmatched_b")
                if unmatched_b_metadata:
                    row_counts['unmatched_b'] = unmatched_b_metadata.get('total_rows', 0)
                    if not timestamp:
                        timestamp = unmatched_b_metadata.get('upload_time')
            ...
            if total_records > 0:
                ...
                logger.debug(f"ReconciliationStorage METADATA NOT FOUND: {recon_id} - no separate files found")
                return None
    ...
    def delete_results(self, recon_id: str) -> bool:
        """Delete all stored results for a reconciliation ID"""
        try:
            ...
            file_suffixes = ['_matched', '_unmatched_a', '_unmatched_b']
            
            for suffix in file_suffixes:
                file_id = f"{recon_id}{suffix}"
                if self.storage.exists(file_id):
                    success = self.storage.delete(file_id)
                    if success:
                        deleted_count += 1
                        logger.debug(f"ReconciliationStorage: Deleted {file_id}")
                    else:
                        all_success = False
                        logger.warning(f"ReconciliationStorage: Failed to delete {file_id}")
            
            if deleted_count > 0:
                logger.info(f"ReconciliationStorage: Successfully deleted {deleted_count} files for reconciliation {recon_id}")
            else:
                logger.debug(f"ReconciliationStorage: No files found to delete for {recon_id}")
            
            return all_success and deleted_count > 0
