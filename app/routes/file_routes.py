...
async def validate_file_name(request: dict):
    """
    Validate if a filename is available - optimized to only load metadata
    """
    try:
...
async def list_files(include_reconciliation_results: bool = True):
    ...
    try:
        ...
        # Reconciliation results are now stored directly in uploaded_files
        # No need for separate handling - they'll appear automatically in the files list
        
        # Filter to show only reconciliation files if needed based on include_reconciliation_results
        if not include_reconciliation_results:
            # Remove reconciliation files if not requested
            files = [f for f in files if f.get("file_source") != "reconciliation"]
        
        all_files = files
        
        # Count reconciliation vs uploaded files
        uploaded_files_count = len([f for f in all_files if f.get("file_source") != "reconciliation"])
        reconciliation_files_count = len([f for f in all_files if f.get("file_source") == "reconciliation"])
        
        logger.info(f"FileRoutes: Successfully listed {uploaded_files_count} uploaded files and {reconciliation_files_count} reconciliation results")
        ...
        response_data = {
            ...
            "summary": {
                ...
                "reconciliation_results": reconciliation_files_count,
        ...
        if include_reconciliation_results:
            message += f" ({uploaded_files_count} uploaded, {reconciliation_files_count} reconciliation results)"
...
async def get_file_info(file_id: str, include_sample: bool = False, sample_rows: int = 10):
    """Get detailed information about a specific file (both uploaded files and reconciliation results)"""
    
    # Both uploaded files and reconciliation results are now in uploaded_files
    if not uploaded_files.exists(file_id):
        raise HTTPException(404, "File not found")
...
async def delete_file(file_id: str):
    """Delete a file (both uploaded files and reconciliation results) from storage"""
    ...
    if not success:
        raise HTTPException(500, f"Failed to delete file {file_id}")
